...
from animateurs.services.flottants import groupes_partages_visibles, groupes_visibles
...
def generer_gabarit_excel(centre_ids: list[int], debut: date, fin: date) -> bytes:
    ...
    evenements = (
        groupes_visibles(Evenement.objects.filter(centre_id__in=centre_ids))
...
def _lignes_officielles(classeur, meta) -> tuple[list[dict], list[dict]]:
    ...
    evenements = {item.id: item for item in groupes_visibles(Evenement.objects.all()).select_related("centre", "groupe").filter(
...
def _lignes_externes(classeur, configuration: dict) -> tuple[list[dict], list[dict]]:
    ...
    groupes = list(groupes_partages_visibles(Groupe.objects.all()))
    centres_auto = {}
    for centre in centres:
...
def previsualiser_classeur(fichier, configuration: dict | None = None) -> dict:
    ...
    evenements = {
        ...
        for item in groupes_visibles(Evenement.objects.all()).select_related("centre", "groupe").filter(
