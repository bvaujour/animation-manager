...
from animateurs.services.meteo_sorties import code_meteo, geocoder_lieux, prevision_sortie


def sortie(date, debut=None, retour=None):
    return Sortie(
        nom="Parc", destination="Parc", date=date,
        meteo_lieu_libelle="Paléopolis", meteo_adresse="03800 Gannat",
        meteo_latitude=Decimal("46.180000"), meteo_longitude=Decimal("3.190000"),
...
class MeteoSortiesServiceTests(SimpleTestCase):
    ...
    def test_geocodage_parse_et_met_en_cache(self):
        brut = {"features": [{"geometry": {"coordinates": [3.19, 46.18]}, "properties": {"name": "Paléopolis", "label": "Route de Bègues 03800 Gannat", "postcode": "03800"}}]}
        with patch("animateurs.services.meteo_sorties._json_externe", return_value=brut) as appel:
            resultat = geocoder_lieux("Paléopolis Gannat")
            self.assertEqual(resultat[0]["latitude"], 46.18)
            self.assertEqual(geocoder_lieux("Paléopolis Gannat"), resultat)
            appel.assert_called_once()

    def test_geocodage_vide_sans_resultat_et_erreur(self):
        with self.assertRaises(ValueError):
            geocoder_lieux("")
        with patch("animateurs.services.meteo_sorties._json_externe", return_value={"features": []}):
            self.assertEqual(geocoder_lieux("Lieu inconnu"), [])
        cache.clear()
        with patch("animateurs.services.meteo_sorties._json_externe", side_effect=RuntimeError), self.assertRaises(RuntimeError):
            geocoder_lieux("Lieu indisponible")
