...
class EffectifsEnfantsPlanningTests(TestCase):
    def setUp(self):
        ...
        self.groupe = Evenement.objects.create(centre=self.centre, nom="Maternels", permanent=True)
        self.url = reverse("api_effectifs_enfants_groupe", args=[self.groupe.id])

    def test_enregistre_lit_et_efface_un_effectif(self):
        response = self.client.post(
            self.url,
            data=json.dumps({"effectifs": [{"date": "2026-07-20", "nombre": 18, "enfants_par_animateur": 8}]}),
            content_type="application/json",
        )
        self.assertEqual(response.status_code, 200)
        self.assertTrue(EffectifEnfantsJour.objects.filter(
            evenement=self.groupe, date=date(2026, 7, 20), nombre=18, enfants_par_animateur=8
        ).exists())

        response = self.client.get(self.url, {"debut": "2026-07-20", "fin": "2026-07-21"})
        self.assertEqual(response.json(), [{"date": "2026-07-20", "nombre": 18, "enfants_par_animateur": 8}])
    ...
    def test_ratio_par_defaut_du_groupe_est_utilise(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(EffectifEnfantsJour.objects.filter(
            evenement=self.groupe, date=date(2026, 7, 20), enfants_par_animateur=12
        ).exists())
    ...
    def test_refuse_un_ratio_invalide(self):
        response = self.client.post(
            self.url,
            data=json.dumps({"effectifs": [{"date": "2026-07-20", "nombre": 18, "enfants_par_animateur": 0}]}),
            content_type="application/json",
        )
        self.assertEqual(response.status_code, 400)
