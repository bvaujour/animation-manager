...
class PlanningSolverTests(TestCase):
    ...
    def test_groupe_permanent_sans_periode_explicitement_liee_est_rempli(self):
        self.groupe.permanent = True
        self.groupe.save(update_fields=["permanent"])
        self.groupe.periodes_scolaires.clear()
    ...
    def test_un_diplome_couvre_le_besoin_de_sa_categorie(self):
        diplome = Qualification.objects.create(nom="Diplômé", est_statut=True)
        bafa = Qualification.objects.create(nom="BAFA", statut=diplome)
        BesoinQualification.objects.create(
            evenement=self.groupe,
...
class PlanningSolverRepartitionGroupesTests(TestCase):
    ...
    def test_couvre_statut_puis_diplome_sans_consummer_le_diplome_precis(self):
        statut = Qualification.objects.create(nom="Diplômé", est_statut=True)
        bafa = Qualification.objects.create(nom="BAFA", statut=statut)
        bpjeps = Qualification.objects.create(nom="BPJEPS", statut=statut)
        groupe_statut, _ = creer_groupe(self.centre, nom="Besoin statut", jours_ouverts=[0], ordre=0)
        groupe_bafa, _ = creer_groupe(self.centre, nom="Besoin BAFA", jours_ouverts=[0], ordre=1)
        BesoinQualification.objects.create(evenement=groupe_statut, qualification=statut, nombre_minimum=1)
        BesoinQualification.objects.create(evenement=groupe_bafa, qualification=bafa, nombre_minimum=1)
        animateur_bafa = Animateur.objects.create(prenom="Alice", nom="BAFA")
        animateur_bpjeps = Animateur.objects.create(prenom="Bruno", nom="BPJEPS")
        animateur_bafa.qualifications.add(bafa)
        animateur_bpjeps.qualifications.add(bpjeps)
        for animateur in (animateur_bafa, animateur_bpjeps):
            rendre_disponible(animateur, SEMAINE_DEBUT, SEMAINE_DEBUT)

        data, status_code = generer_planning_auto({"debut": SEMAINE_DEBUT.isoformat()})

        self.assertEqual(status_code, 200)
        self.assertEqual(data["qualifications_manquantes"], 0)
        self.assertTrue(Affectation.objects.filter(evenement=groupe_statut, animateur=animateur_bpjeps).exists())
        self.assertTrue(Affectation.objects.filter(evenement=groupe_bafa, animateur=animateur_bafa).exists())
