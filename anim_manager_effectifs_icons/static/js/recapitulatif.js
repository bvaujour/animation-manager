document.addEventListener("DOMContentLoaded", () => {
    ...
    pickerRoot?.addEventListener("week-picker:ready", () => {
        const periods = picker?.periods || [];
        if (!periods.length) return;
        const persisted = WeekPicker.getPersistedDate();
        const remembered = periods.find((period) => period.debut <= persisted && persisted <= period.fin);
        ...
        selectPeriod(remembered || current || next || periods.at(-1));
    });
