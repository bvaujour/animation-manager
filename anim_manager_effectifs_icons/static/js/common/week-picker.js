(function () {
    ...
    const STORAGE_KEY = "animation-manager-selected-week-date-v1";
    let sharedPeriodsRequest = null;
    ...
    function loadSharedPeriods() {
        ...
        return sharedPeriodsRequest;
    }

    function getPersistedDate() {
        try {
            const value = String(localStorage.getItem(STORAGE_KEY) || "");
    ...
    function persistDate(date, source = null) {
        const value = String(date || "").slice(0, 10);
        if (!/^\d{4}-\d{2}-\d{2}$/.test(value)) return;
        try {
            localStorage.setItem(STORAGE_KEY, value);
        } catch {
            // Le sélecteur reste fonctionnel même si le stockage est désactivé.
        }
        allInstances.forEach((picker) => {
            if (picker === source || picker.mode !== "single") return;
    ...
    class WeekPicker {
        constructor(root, options = {}) {
            ...
            this.activeDate = String(
                options.activeDate
                || root.dataset.weekPickerActiveDate
                || (this.mode === "single" ? getPersistedDate() : "")
            ).slice(0, 10);
            this.ready = false;
            this.defaultCurrent = options.defaultCurrent ?? root.dataset.weekPickerDefaultCurrent !== "false";
            this.genericNavigation = options.genericNavigation
                ?? root.dataset.weekPickerGenericNavigation === "true";
        ...
        async load() {
            if (!this.tree) return;
            try {
                const periods = await loadSharedPeriods();
        ...
        createOption(period) {
            ...
            if (this.mode === "multiple") {
                ...
                option.addEventListener("click", () => this.selectPeriod(period));
            }
        ...
        navigateGeneric(direction) {
            ...
            const currentIndex = reference
                ? this.periods.findIndex((period) => Number(period.id) === Number(reference.id))
                : -1;
            const targetIndex = currentIndex >= 0
                ? currentIndex + direction
                : (direction > 0 ? 0 : this.periods.length - 1);
            const target = this.periods[targetIndex];
            if (target) this.selectPeriod(target);
        }

        goToCurrentGeneric() {
            if (!this.periods.length) return;
            const today = localIsoDate();
            const target = this.periods.find((period) => isCurrentPeriod(period, today))
                || this.periods.find((period) => String(period.debut || "") > today)
                || this.periods.at(-1);
            if (target) this.selectPeriod(target);
        }
    ...
    window.WeekPicker = {
        ...
        groupPeriods,
        getPersistedDate,
        setPersistedDate: (date) => persistDate(date),
    };
})();
