...
function libelleDate(dateStr)
	...
	function iconeEffectif(type)
	{
		const icones = {
			...
			encadrement: `<svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M8 11a3 3 0 1 0 0-6 3 3 0 0 0 0 6Zm8 0a3 3 0 1 0 0-6 3 3 0 0 0 0 6ZM8 13c-3.31 0-6 1.79-6 4v2h8.35A5.96 5.96 0 0 1 10 17c0-1.54.58-2.94 1.53-4H8Zm8 0c-.57 0-1.13.05-1.65.16A4 4 0 1 0 20 16.82C19.9 14.7 18.16 13 16 13Zm0 2a2 2 0 1 1 0 4 2 2 0 0 1 0-4Z"/></svg>`,
		};
		...
		cellules.forEach((cellule) =>
		{
			...
			badge.innerHTML = `
				<span class="planning-effectif-details">
					...
					<span class="planning-effectif-line planning-animateurs-compteur" title="Animateurs affectés / nécessaires">${iconeEffectif("encadrement")}<strong>${animateursAffectes}/${animateursNecessaires}</strong></span>
				</span>`;
			badge.querySelectorAll("[data-effectif-inline]").forEach((bouton) =>
			{
				// FullCalendar commence sa sélection dès pointerdown/mousedown, avant
				// l'événement click. On coupe donc la propagation dès le début de
				// l'interaction afin que le bouton garde toujours la priorité.
	...
	boutonSupprimerAffectation?.addEventListener("click", async () =>
	{
		...
		try
		{
			await apiFetch(`/api/affectations/${contexteHorairesAffectation.affectation.id}/`, { method: "DELETE" });
			PlanningData.invalidateWeekEvents();
			contexteHorairesAffectation.affectation.remove();
			fermerModal(modalHorairesAffectation);
